...
/// connection's story under one id (`[3921 5004ms]`): the destination, the process, the
/// outbound it dials and — when it fails — an info-level failure line. The tracker joins
...
public struct FailedConnections: Sendable, Equatable {
    ...
    /// Connections opened per exit, once per connection id, at its first outbound dial line.
    private var openedCounts: [String: Int] = [:]
    /// Connections that failed per exit, at the failure line (blocked excluded).
    ...
    /// Whether an outbound dial line has been seen since the last `clear()` — while false,
    /// `opened` stays nil for every exit (log detail *Problems*).
    ...
    public static func isInteresting(_ message: String) -> Bool {
        ...
        return message.contains("connection to ") || message.contains("found process path")
            || message.contains(" => ") || message.contains("dns: exchange ")
    }

    /// Feeds one line (the message after the timestamp, i.e. `SingBoxLog.message(of:)`,
    /// with the level still in front or not — both shapes are accepted; ANSI escapes must
    /// already be stripped by the relay).
    public mutating func ingest(_ line: String, level: LogLevel, at date: Date = Date()) {
            ...
            ?? Self.value(after: "inbound packet connection to ", in: rest)
        {
            ...
            remember(id) { $0.processPath = Self.stripUserSuffix(path) }
            ...
            // The block-list reject/predefined match — not seen in the 1.13.19 live log,
            // kept as it was (verify); `BlockCounter` reads the same shape.
            if (target == "reject" || target == "predefined"),
                rest.contains("rule_set=\(SingBoxConfigGenerator.blockListTag)")
            {
                sawMatchLine = true
                record(id, reason: .blocked, exitOverride: "", at: date)
            }
        } else if rest.hasPrefix("outbound/"), let tag = Self.outboundTag(in: rest),
            let host = Self.value(after: "outbound connection to ", in: rest)
                ?? Self.value(after: "outbound packet connection to ", in: rest)
        {
            // `outbound/<type>[<tag>]: outbound connection to <host>:<port>` — the exit for
            // this id, at the first dial; the line repeats while sing-box connects. The
            // host here replaces any fake-ip the inbound line gave (sing-box resolves it
            // back before dialling), so no fake-ip map is needed.
            sawMatchLine = true
            let exit = Self.exitID(fromOutboundTag: tag)
            // Opened counts once per connection id, at the first outbound dial line.
            if pending[id]?.exit == nil { openedCounts[exit, default: 0] += 1 }
            remember(id) {
                $0.exit = exit
                $0.host = Self.stripPort(host)
            }
        } else if rest.hasPrefix("connection:"), let failure = Self.failedConnection(in: rest) {
            // `connection: open connection to <host> using outbound/<type>[<tag>]: <error>`
            // — an info-level line; host and exit come from the line itself, so a failure
            // is recorded even when its prelude was not seen (log detail Problems).
            let exit = Self.exitID(fromOutboundTag: failure.tag)
            remember(id) { $0.host = Self.stripPort(failure.hostPort) }
            record(
                id, reason: Self.classify(failure.error, exit: exit), exitOverride: exit, at: date)
        }
    }
    ...
    /// The tag inside a leading `outbound/<type>[<tag>]:`; nil for any other line.
    static func outboundTag(in rest: String) -> String? {
        guard let slash = rest.range(of: "outbound/"),
            let open = rest[slash.upperBound...].firstIndex(of: "["),
            let close = rest[open...].firstIndex(of: "]")
        else { return nil }
        return String(rest[rest.index(after: open)..<close])
    }

    /// `connection: open connection to <host> using outbound/<type>[<tag>]: <error>` (or
    /// `open packet connection to` for UDP) → its pieces; nil for any other line.
    static func failedConnection(in rest: String) -> (hostPort: String, tag: String, error: String)?
    {
        guard
            let failure = value(after: "open connection to ", in: rest)
                ?? value(after: "open packet connection to ", in: rest),
            let usingRange = failure.range(of: " using outbound/")
        else { return nil }
        let hostPort = String(failure[..<usingRange.lowerBound])
        let afterSlash = failure[usingRange.upperBound...]
        guard let open = afterSlash.firstIndex(of: "["),
            let close = afterSlash[open...].firstIndex(of: "]"),
            let colon = afterSlash[close...].range(of: ": ")
        else { return nil }
        let tag = String(afterSlash[afterSlash.index(after: open)..<close])
        let error = String(afterSlash[colon.upperBound...])
        return (hostPort, tag, error)
    }

    /// `router: found process path: <path>, user: <name>` → `<path>`; the suffix sing-box
    /// appends.
    static func stripUserSuffix(_ path: String) -> String {
        guard let comma = path.range(of: ", user: ") else { return path }
        return String(path[..<comma.lowerBound])
    }
